      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex justify-between mt-4 gap-2">
            <button type="button" className="px-6 py-2 rounded border border-black hover:bg-gray-200 transition-colors" onClick={onClose}>Cancelar</button>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm bg-black/20" onClick={() => setShowDeleteConfirm(false)}>
            <div className="bg-white rounded-xl shadow-lg p-8 relative w-full max-w-sm" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex justify-end gap-2">
                <button className="px-4 py-2 rounded border border-black hover:bg-gray-200" onClick={() => setShowDeleteConfirm(false)}>Cancelar</button>
                <button className="px-4 py-2 rounded bg-red-600 text-white font-semibold hover:bg-red-700" onClick={() => { setShowDeleteConfirm(false); onDelete(); }}>Deletar</button>
              </div>
            </div>
          </div>
        )}
